...
export const fetchModels = async (
  ...
  switch (providerName) {
    ...
    case LLMProviderName.DASHSCOPE:
      return fetchDashscopeModels({
        api_base: formValues.api_base,
        api_key: formValues.api_key,
        provider_id: formValues.id,
        signal,
...
/**
 * Fetches chat models from an Alibaba Bailian (DashScope) workspace via its
 * OpenAI-compatible `/models` endpoint, with per-model metadata from the
 * LiteLLM cost map.
 */
export const fetchDashscopeModels = async (
  params: DashscopeFetchParams
): Promise<{ models: ModelConfiguration[]; error?: string }> => {
  const apiBase = params.api_base;
  if (!apiBase) {
    return { models: [], error: "API Base is required" };
  ...
  try {
    const response = await fetch("/api/admin/llm/dashscope/available-models", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.warn(
          "Failed to parse DashScope model fetch error response",
          jsonError
        );
      }
      return { models: [], error: errorMessage };
    ...
    const data: DashscopeModelResponse[] = await response.json();
    const models: ModelConfiguration[] = data.map((modelData) => ({
      name: modelData.name,
      display_name: modelData.display_name,
      is_visible: true,
