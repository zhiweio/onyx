...
interface ActivityToolLineProps {
  toolCall: ToolCallState;
}

export default function ActivityToolLine({ toolCall }: ActivityToolLineProps) {
  ...
  if (!expandable) {
    return <div className="w-full min-w-0 max-w-full overflow-hidden py-0.5">{header}</div>;
  }
