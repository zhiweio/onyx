...
#### TTS 转换器
```bash
node scripts/tts-converter.js "要转换的文本" --voice en-US-AriaNeural --rate +10% --output output.mp3
```
...
Sandbox 镜像已预装 `node-edge-tts` 和 `commander`。直接运行 `scripts/tts-converter.js`，不要再 `npm install`。
