...
description: Screen new suppliers from Qichacha MCP registry and dishonest lists. Use for 新供应商筛选.
required-mcp:
  - qcc-company
  - qcc-risk
---
...
Official skill: https://agent.qcc.com/skill/v1/supply/new-supplier-screening-qcc/SKILL.md

## Task

Screen each named candidate. Stop on an ambiguous name.

## Phases

1. **anchor** — `get_company_by_query` then
   `get_company_registration_info` per name.
2. **scan** — `get_company_risk_scan`. Hard fail on current 失信 /
   吊销 / 注销.
3. **write** — Screening table. One row per candidate.
