...
description: Assess recovery odds from Qichacha MCP finance and execution data. Use for 追偿 or 财产保全.
required-mcp:
  - qcc-risk
  - qcc-company
optional-mcp:
...
Official skill: https://agent.qcc.com/skill/v1/legal/debt-recovery-assessment-qcc/SKILL.md

## Task

List executable assets and unpaid execution.

## Phases

1. **anchor** — `get_company_registration_info` and `get_financial_data`.
2. **scan** — `get_company_risk_scan`.
3. **drill** — `get_terminated_cases`, `get_judicial_auction`,
   `get_dishonest_info`, `get_equity_freeze`, `get_chattel_mortgage_info`.
4. **people** — If `qcc-executive` is up, `get_executive_risk_scan` for
   法定代表人 and 实际控制人.
5. **write** — Recovery memo. Quote unpaid amounts as returned.
