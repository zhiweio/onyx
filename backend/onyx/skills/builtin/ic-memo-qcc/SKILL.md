---
name: ic-memo-qcc
description: Draft an investment-committee memo from Qichacha MCP facts. Use for IC memo, 投委, or 投资备忘.
required-mcp:
  - qcc-company
  - qcc-risk
  - qcc-operation
...
Official skill: https://agent.qcc.com/skill/v1/invest/ic-memo-qcc/SKILL.md

## Task

Facts first. Separate judgment from tool output.

## Phases

1. **anchor** — `get_company_registration_info`, `get_shareholder_info`,
   `get_actual_controller`.
2. **scan** — `get_company_risk_scan`, then drill hits.
3. **ops** — `get_financing_records`, `get_bidding_info`.
4. **ip** — If `qcc-ipr` is up, `get_patent_info` and `get_trademark_info`.
5. **write** — IC memo. Label judgment as judgment.
