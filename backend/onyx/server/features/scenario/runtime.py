...
def render_report_template_section(template: ReportTemplate) -> list[str]:
    ...
    if template.kind is ReportTemplateKind.DOCX:
        ...
        lines.extend(
            [
                "",
                "### Report format: Word reference",
                "",
                f"Use `{asset_path}` as the layout and style reference. "
                "Follow its sections, tables, headers and visual style. "
                "Write the report from the evidence. Do not invent a "
                "fill-in schema and do not substitute a markdown report.",
                "",
                "```bash",
